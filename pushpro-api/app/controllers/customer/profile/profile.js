...
const getCustomerProfileById = catchAsyncError(async (req, res) => {
  try {
    const customerId = req.params.id;
    const contractorId = req.user.id;
    const customer = await UserServices.getCustomer({ id: customerId });
    if (!customerId || !customer) {
      throw new ValidationError("Customer Not Found");
    }
    const relation = await UserServices.checkRelation(customerId, contractorId);
    if (!relation) {
      throw new UnauthorizedError("Not Authorized. Not your Customer");
    }
    const data = {
...
const changePassword = catchAsyncError(async (req, res) => {
  try {
    ...
    const [updatedRowCount] = await db.Users.update(
      {
