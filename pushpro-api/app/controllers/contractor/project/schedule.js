...
const checkAccess = (userId, contractorId) => {
  if (userId !== contractorId) {
    throw new UnauthorizedError("Access Denied. Not Your Schedule.");
  }
};
...
const getDashboardSchedules = catchAsyncError(async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data: data,
      message: "Contractor Schedules Fetched Successfully",
      errors: []
    });
  } catch (error) {
...
const getContractorSchedules = catchAsyncError(async (req, res) => {
  try {
    const id = req.params.id;
    const userId = req.user.id;
    const project = await ProjectServices.getProjectById(id);
    if (!project) throw new ValidationError("Project Not Found");
    const contractor = await UserServices.getContractorById({ id: userId });
    if (!contractor) throw new ValidationError("Contractor Not Found");
    const schedules = await ProjectScheduleServices.getAllSchedules({
      contractor_id: userId
    });
    checkAccess(userId, project.contractor_id || id);
    const schedulesArray = await Promise.all(
    ...
    const serverError = new ServerError(
      `Cannot Get Contractor Schedule Details : ${error.message}`
    );
    res.status(serverError.status).json(serverError.serializeError());
  }
});
...
const createProjectSchedule = catchAsyncError(async (req, res) => {
  try {
    ...
    const notification = await NotificationService.createNotification({
      ...
      link: `${process.env.BASE_URL_CUSTOMER}/project-schedule/${project.id}/`,
      is_read: false
    });
