...
const Cover = () => {
  ...
  const { handleAutoCloseSuccess } = service;
    ...
    try {
      ...
      if (validationErrors?.response) {
        if (validationErrors?.response?.status === 422 || 423 || 424) {
          setError({
            verificationCodeExp: validationErrors?.response?.data?.errors[0]
          });
          setTimeout(() => {
            router.push(`/login/${currentContractor}`);
          }, 2100);
                  ...
                  <div className="p-5">
                    ...
                    <div className="row gy-3 mt-4">
                      ...
                      {/* {error && <Alert variant="danger">{error}</Alert>}
                                                    {passwordChanged && <Alert variant="success">{passwordChanged}</Alert>} */}
