...
export function addOrRemoveCss(cssCode) {
  if (document.head) {
    document.head.removeChild(
      document.head.querySelector(`style[data-name="dynamic-bootstrap"]`)
    );
  }
...
export const HorizontalClick = (actionfunction) => {
  ...
  actionfunction({
    ...
    dataMenuStyles: localStorage.ynexdarktheme
      ? "dark"
...
export const MenuHover = (actionfunction) => {
  setTimeout(() => {
    ...
    ulElement &&
      ...
      (ulElement.style.display = ulElement.classList.contains("child1")
        ? "block"
        : "none"));
  }, 100);
...
export const DoubletFn = (actionfunction) => {
  const theme = store.getState();
  const menuNochildElement = document.querySelectorAll(
...
export const primaryColor3 = (actionfunction) => {
  ...
  localStorage.setItem("dynamiccolor", "161, 90, 223");
...
export const Reset = (actionFunction) => {
  ...
  // Clear local storage
  const keepItems = ["user_data", "token"];
...
export const Reset1 = (actionfunction) => {
  if (document.querySelector("[data-styled-js]")) {
    document.querySelector("[data-styled-js]").remove();
  }
  const theme = store.getState();
...
export const LocalStorageBackup = (actionfunction) => {
  localStorage.ynexrtl ? Rtl(actionfunction) : Ltr(actionfunction);
