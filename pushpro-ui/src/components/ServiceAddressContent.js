...
const ServiceAddressContent = ({
  ...
  useEffect(() => {
    ...
    const uuid = userDataObject ? userDataObject.user_uuid : null;
    if (token) {
      setToken(token);
    }
    if (uuid) {
  ...
  const validationSchema = Yup.object().shape({
    ...
    zipCode: Yup.string().required(t("serviceAddressValidations.zipCode"))
  });
  ...
  const fetchAddress = async () => {
    try {
      const response = await service.get(`/addressBook`);
  ...
  const createMyAddress = async () => {
    try {
      const response = await service.post("/create-address", {
        address_line_1: formData.addressLine1,
        address_line_2: formData.addressLine2,
        city: formData.city,
        state: formData.state,
  ...
  const updateMyAddress = async (addressId) => {
    try {
      await validationSchema.validate(formData, { abortEarly: false });
      const response = await service.patch(`/update-address/${addressId}`, {
        address_line_1: formData.addressLine1,
        address_line_2: formData.addressLine2,
        city: formData.city,
        state: formData.state,
